...
SUPER + M	🚀 Hyprland	Spotify Dropdown Scratchpad (Vim)	Spotify suspenso com navegação Vim (j/k scroll, g/G topo/fim, f hints, / busca).	spotify musica player dropdown scratchpad vim j k f buscar	/home/lan/.config/hypr/scripts/spotify-toggle.sh
...
SUPER + ; ou SUPER + .	🚀 Hyprland	Seletor de Emojis Apex (PT-BR)	Busca inteligente de emojis em português, gírias BR, categorias WhatsApp e auto-type.	emoji pt br carinha joinha coracao whatsapp rofi super . super ;	/home/lan/.config/hypr/scripts/RofiEmoji.sh
...
SUPER + CTRL + A	🎭 Hyprland	Seletor de Animações & Transições	Abre menu visual para alternar entre presets de animação (macOS Glass, Fast, Minimal, etc).	animacoes animacao transicoes transicao macos apple fluid rofi hyprland	/home/lan/.config/hypr/scripts/Animations.sh
