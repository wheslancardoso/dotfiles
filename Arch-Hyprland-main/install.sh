...
NOTE: If you are installing on a VM, ensure to enable 3D acceleration else Hyprland may NOT start!" \
    ...
    echo "⚡ Modo de instalação automatizada / Preset ativado." | tee -a "$LOG"
fi
...
if ! command -v yay &>/dev/null && ! command -v paru &>/dev/null; then
    if [ -n "$aur_helper" ]; then
        echo "${INFO} - Using pre-configured AUR helper: $aur_helper" | tee -a "$LOG"
    elif [ "$non_interactive" == "true" ]; then
        aur_helper="yay"
        echo "${INFO} - Defaulting to yay in non-interactive mode" | tee -a "$LOG"
    else
...
if [ "$nvidia_detected" == "true" ]; then
    options_command+=(
        "nvidia" "Do you want script to configure NVIDIA GPU?" "${nvidia:-OFF}"
        "nouveau" "Do you want Nouveau to be blacklisted?" "${nouveau:-OFF}"
    )
fi
...
if [ "$input_group_detected" == "true" ]; then
    options_command+=(
        "input_group" "Add your USER to input group for some waybar functionality?" "${input_group:-OFF}"
    )
fi
...
if ! check_services_running; then
    options_command+=(
        "sddm" "Install & configure SDDM login manager?" "${sddm:-OFF}"
        "sddm_theme" "Download & Install Additional SDDM theme?" "${sddm_theme:-OFF}"
    )
fi

# Add the remaining static options
options_command+=(
    "gtk_themes" "Install GTK themes? (required for Dark/Light function)" "${gtk_themes:-OFF}"
    "bluetooth" "Do you want script to configure Bluetooth?" "${bluetooth:-OFF}"
    "thunar" "Do you want Thunar file manager to be installed?" "${thunar:-OFF}"
    "quickshell" "Install quickshell for Desktop-Like Overview?" "${quickshell:-OFF}"
    "xdph" "Install XDG-DESKTOP-PORTAL-HYPRLAND (for screen share)?" "${xdph:-OFF}"
    "zsh" "Install zsh shell with Oh-My-Zsh?" "${zsh:-OFF}"
    "pokemon" "Add Pokemon color scripts to your terminal?" "${pokemon:-OFF}"
    "rog" "Are you installing on Asus ROG laptops?" "${rog:-OFF}"
    "dots" "Download and install pre-configured KooL Hyprland dotfiles?" "${dots:-OFF}"
)
...
if [ "$non_interactive" == "true" ]; then
    ...
    echo "👌 ${OK} Opções selecionadas (Modo Automático/Preset): $selected_options" | tee -a "$LOG"
else
    while true; do
