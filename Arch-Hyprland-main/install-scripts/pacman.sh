...
# Enable [multilib] repository (crucial for Steam, Wine, Proton, 32-bit drivers)
if [ -f "$pacman_conf" ] && ! grep -q "^\[multilib\]" "$pacman_conf"; then
    echo -e "${NOTE} Enabling ${MAGENTA}[multilib]${RESET} repository for 32-bit gaming & Proton..." 2>&1 | tee -a "$LOG"
